...

{
...

    {
        ...
        /// Shows a large skill activation popup above the correct fighter (CRITICAL!, BURN!, SHIELD!, etc.).

        /// Faded DoT messages and unknown skills are ignored.
...

        {
            ...
            if (!FormatSkillPopupMessage(combatEvent.message, out var popupText))
            ...
            var targetRig = ResolveSkillPopupTarget(popupText);
...

            {
                ...
                Debug.LogWarning(
                    ...
                    $"[FloatingCombatTextSpawner] Cannot spawn skill popup '{popupText}': target FighterRig is missing.");
            ...
            SpawnSkillPopupForFighter(targetRig, popupText, combatEvent.eventType);
        ...
        /// Maps BattleManager SkillTriggered messages to short HUD popup labels.

        /// Returns false for fade/end messages and skills not yet implemented (e.g. DODGE).

        /// </summary>

        private static bool FormatSkillPopupMessage(string eventMessage, out string popupText)

        {
            ...
            // DoT ending is logged but not a skill activation popup.

            if (eventMessage == "Burn faded." || eventMessage == "Poison faded.")
            ...
            if (eventMessage.StartsWith("SHIELD BLOCKED") || eventMessage.StartsWith("SHIELD ABSORBED"))
        ...
        /// Player attack debuffs appear above the enemy; defensive/heal skills appear above the player.

        /// </summary>

        private FighterRig ResolveSkillPopupTarget(string popupText)
        ...
        /// Spawns one skill popup instance using the skill-specific animation on FloatingCombatText.

        /// </summary>

        private void SpawnSkillPopupForFighter(FighterRig fighterRig, string popupText, CombatEventType eventType)

        {
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn skill popup '{popupText}' ({eventType}): FighterRig reference is missing.");
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn skill popup '{popupText}' ({eventType}): Canvas not found in scene.");
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn skill popup '{popupText}' ({eventType}): FloatingTextLayer is missing.");
...

            {
                ...
                Debug.LogWarning(
                    ...
                    $"[FloatingCombatTextSpawner] Cannot spawn skill popup '{popupText}' ({eventType}): failed to resolve spawn position for {fighterRig.name}.");
            ...
            instance.InitializeSkillPopup(popupText);
            ...
            Debug.Log(
                ...
                $"[FloatingCombatTextSpawner] Spawned skill popup: {popupText} at anchoredPosition {anchoredPosition}{fallbackNote}");
...

        {
...

            {
...

                {
                    ...
                    SpawnCritForFighter(enemyFighterRig, text, combatEvent.eventType);
        ...
        /// Spawns one combined crit popup: "CRIT!" line above the damage number in yellow bold text.

        /// </summary>

        private void SpawnCritForFighter(FighterRig fighterRig, string damageLine, CombatEventType eventType)

        {
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn crit '{damageLine}' ({eventType}): FighterRig reference is missing.");
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn crit '{damageLine}' ({eventType}): Canvas not found in scene.");
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn crit '{damageLine}' ({eventType}): FloatingTextLayer is missing.");
...

            {
                ...
                Debug.LogWarning($"[FloatingCombatTextSpawner] Cannot spawn crit '{damageLine}' ({eventType}): failed to resolve spawn position for {fighterRig.name}.");
            ...
            instance.InitializeCrit(damageLine, CritColor);
            ...
            Debug.Log(
                ...
                $"[FloatingCombatTextSpawner] Spawned crit floating text: CRIT! {damageLine} at anchoredPosition {anchoredPosition}{fallbackNote}");
