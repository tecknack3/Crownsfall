...
namespace Crownsfall.Combat.UI
{
    ...
    public class WaveBannerUI : MonoBehaviour
    {
        ...
        private static readonly Color DefaultBackdropColor = new Color(0.04f, 0.05f, 0.1f, 0.85f);
        private static readonly Color DefaultTitleColor = Color.white;
        private static readonly Color DefaultSubtitleColor = Color.white;
        private static readonly Color BossBackgroundColor = new Color(0.4f, 0.05f, 0.05f, 0.85f);
        private static readonly Color BossTitleColor = new Color(1f, 0.84f, 0f, 1f);

        private const float TitleFontSize = 52f;
        private const float SubtitleFontSize = 32f;
        private const float BossExtraHoldSeconds = 0.75f;
        ...
        private Color normalSubtitleColor;
        ...
        public IEnumerator ShowWaveBanner(int waveNumber, string enemyName, bool isBoss)
        {
            ...
            if (isBoss)
            {
                Debug.Log($"Showing Boss Banner: Wave {waveNumber} / {displayName}");
                SetText(titleText, $"⚔ BOSS WAVE {waveNumber} ⚔");
                SetText(subtitleText, displayName);
                if (titleText != null)
                {
                    titleText.fontSize = TitleFontSize;
                    titleText.color = BossTitleColor;
                }
                if (subtitleText != null)
                {
                    subtitleText.fontSize = SubtitleFontSize;
                    subtitleText.color = normalSubtitleColor;
                }
                if (_backdropImage != null)
                {
                    _backdropImage.color = BossBackgroundColor;
                }
                effectiveHold = holdDuration + BossExtraHoldSeconds;
            }
            else
            {
                Debug.Log($"Showing Wave Banner: Wave {waveNumber} / {displayName}");
            ...
            if (effectiveHold > 0f)
            {
                yield return new WaitForSeconds(effectiveHold);
            }
        ...
        {
            normalBackgroundColor = _backdropImage != null ? _backdropImage.color : DefaultBackdropColor;
            normalTitleColor = titleText != null ? titleText.color : DefaultTitleColor;
            normalSubtitleColor = subtitleText != null ? subtitleText.color : DefaultSubtitleColor;
        }
        ...
        private void RestoreNormalColors()
        {
            if (_backdropImage != null)
            {
                _backdropImage.color = normalBackgroundColor;
            }

            if (titleText != null)
            {
                titleText.color = normalTitleColor;
            }
